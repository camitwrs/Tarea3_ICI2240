...
typedef struct HashMapPair HashMapPair;
...
HashMapPair * createPair( char * key,  void * value) {
...
int is_equal_hashmap(void* key1, void* key2){
...
void insertMap(HashMap * map, char * key, void * value) {
    ...
    while (map->buckets[idx] != NULL && map->buckets[idx]->key != NULL){
        if(is_equal_hashmap(map->buckets[idx]->key, key) == 1) return; //el dato ya existe
...
void enlarge(HashMap * map) {
    ...
    HashMapPair ** oldBucket = map->buckets;
    
    long oldCapacity = map->capacity/2;

    map->buckets = (HashMapPair **)malloc(sizeof(HashMapPair *) * map->capacity);
    memset(map->buckets, 0, map->capacity * sizeof(HashMapPair *));
    map->size = 0;
    long i;
...
HashMap * createMap(long capacity) {
    ...
    new->buckets = (HashMapPair **) calloc (capacity,sizeof(HashMapPair *));
    new->size = 0;
...
void eraseMap(HashMap * map,  char * key) {    
    long idx = hash(key, map->capacity);
    while (map->buckets[idx] != NULL && is_equal_hashmap(map->buckets[idx]->key, key) == 0)
        idx = (idx + 1) % map->capacity;
